...
const height = 480;
function ready() {
    __WEBPACK_IMPORTED_MODULE_0_d3__["json"]("data/factil.json", main);
}
...
function get_links(data) {
    const nodes = get_nodes(data).map((x) => x.name);
    const convert = (d) => {
        return { source: nodes.indexOf(d.source),
...
function main(error, data) {
    const svg = __WEBPACK_IMPORTED_MODULE_0_d3__["select"]("#chart")
        .append("svg")
        .attr("width", width)
        .attr("height", height);
    ...
    console.log(links);
    const d3cola = cola.d3adaptor(__WEBPACK_IMPORTED_MODULE_0_d3__)
        .linkDistance(120)
        .avoidOverlaps(true)
        .size([width, height]);
    d3cola
